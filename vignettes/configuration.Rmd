...
* `"vsc.completionsLoadNamespaces" = TRUE`: Whether completion may load an otherwise unloaded package namespace
* `"vsc.completionsForceNamespacePromises" = TRUE`: Whether completion may force promises in search-path environments and namespaces, in particular lazy-loaded objects
* `"vsc.completionsFunctionArgumentSpaces" = FALSE`: Whether function-argument completion inserts `name = ` rather than `name=`
...
* `"vsc.evaluateActiveBindings" = FALSE`: Whether to evaluate active bindings for the variables view and completions provider.
...
* `"vsc.linkSrcrefLocations" = TRUE`: Whether to create links to source locations for functions with srcref attributes
* `"vsc.previewPromises" = FALSE`: Whether to use the value that a promise would currently evaluate to in the variables view and for completions.
